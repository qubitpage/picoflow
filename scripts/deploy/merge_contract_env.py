...
FRAGMENT = Path('/tmp/picoflow-contracts.env')
TARGETS = [Path('/opt/picoflow/code/.env'), Path('/opt/picoflow/code/.env.production')]
KEYS = {'BOND_VAULT_ADDR', 'REPUTATION_REGISTRY_ADDR', 'METADATA_LOGGER_ADDR'}
...
for target in TARGETS:
    ...
    for line in original.splitlines():
        ...
        if key not in KEYS:
            keep.append(line)
    target.with_suffix(target.suffix + '.bak-contracts').write_text(original, encoding='utf-8')
    merged = '\n'.join(keep).rstrip()
    if merged:
        merged += '\n\n'
    merged += '# Deployed Vyper contracts on Arc Testnet\n' + '\n'.join(fragment_lines).rstrip() + '\n'
    target.write_text(merged, encoding='utf-8')
    print(f'{target}: configured {len(fragment_keys)} contract addresses')
